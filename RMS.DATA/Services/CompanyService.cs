...
namespace RMS.DATA.Services
{
    internal class CompanyService : IServiceExtended<Company, int, string>
    {
        ...
        public async Task<IEnumerable<Company>> CreateListOfEntitiesAsync(IEnumerable<Company> list)
        {
            using var connection = new SqliteConnection(dbConfig.Name);
        ...
        public async Task UpdateListOfEntitiesAsync(IEnumerable<Company> items)
        {
            using var connection = new SqliteConnection(dbConfig.Name);
